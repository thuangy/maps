...
def find_closest(location, centroids):
    ...
    distances = {tuple(c): distance(location, c) for c in centroids}
    for key, value in distances.items():
        if value == min(distances.values()):
            return key

    #distances = [distance(location, c) for c in centroids]
...
def group_by_centroid(restaurants, centroids):
    ...
    """
    pairs = [[centroids.index(find_closest(restaurant_location(r), centroids)), r] for r in restaurants]
    return group_by_first(pairs)

    #pairs = []
    #for r in restaurants:
        #index = centroids.index(find_closest(restaurant_location(r), centroids))
        #pairs.append([index, r])
    #return group_by_first(pairs)
...
def find_centroid(restaurants):
    ...
    cen_long = mean([restaurant_location(r)[0] for r in restaurants])
    cen_lat = mean([restaurant_location(r)[1] for r in restaurants])
    return [cen_long, cen_lat]
...
def k_means(restaurants, k, max_updates=100):
    ...
    while old_centroids != centroids and n < max_updates:
        ...
        centroids = [find_centroid(g) for g in grouped_rest]
        n += 1
    return centroids
...
def best_predictor(user, restaurants, feature_fns):
    ...
    for f in feature_fns:
        ...
        predictors[pred] = r_squared
    for p, r in predictors.items():
        if r == max(predictors.values()):
            return p
...
def rate_all(user, restaurants, feature_functions):
    ...
    user_restaurants = user_reviewed_restaurants(user, restaurants)
    predictions = {}
    for name in restaurants:
        if name in user_restaurants:
            predictions[name] = user_rating(user, name) 
        else:
            predictions[name] = predictor(RESTAURANTS[name])
    return predictions
...
def search(query, restaurants):
    ...
    #seq = []
    #for name in restaurants:
        #if query in restaurant_categories(name):
            #seq.append(name)
    #return seq
